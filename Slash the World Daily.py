...
async def report(ctx):
    if ctx.ctx.content[10:] == '':
        await ctx.send(embed=discord.Embed(title="I'm gonna need a little more detail than that i'm afraid"
        ...
        ctx_content = str(ctx.ctx.content)
        author_id = ctx.author.id
        print(f'Report Created by {client.get_user(ctx.author.id)}. Content: {ctx_content}')
        embed = discord.Embed(title='Information', description='_ _', colour=discord.Colour.red())
        embed.set_author(name=ctx.author.name, icon_url=ctx.author.avatar_url)
        embed.add_field(name='Report Content:', value=f'{ctx_content}', inline=False)
        embed.add_field(name='Invoker ID:', value=f'{author_id} | {client.get_user(ctx.author.id)}', inline=False)
        ...
        embed.set_footer(text=f"\nReport generated on behalf of: {ctx.author.name} • "
...
"""
@slash.slash(name='Reply', aliases=reply_aliases, description='Used to reply to reports. Exclusive to bot owner. F')
async def reply(ctx):
    await ctx.send('Sorry. The command you were looking for has been deprecated due to privacy concerns.')
"""

"""
@slash.slash(name='getmtime', aliases=getmtime_aliases,
                description='Used to get the last modified date of the python file')
async def getmtime(ctx):
    print(f'getmtime requested by: {ctx.author.name}')
    await ctx.send(lastmfunction())
"""

"""
@slash.slash(name='Temp',
                description='Used')
async def temp(ctx):
    print(f'temp requested by: {ctx.author.name}')
    try:
        psutilsensors_temperatures = psutil.sensors_temperatures()
    except:
        psutilsensors_temperatures = 'Not available'
    await ctx.send(psutilsensors_temperatures)
"""
...
async def help(ctx):
    print(f'help requested by: {ctx.author.name}')
...
@slash.slash(name='info',
            description='Used to see stats and info about the bot hosting service.', guild_ids = guild_ids)
async def info(ctx):
    print(f'info requested by: {ctx.author.name}')
    try:
...
async def reward(ctx,
                 ...
                 limit = 7):
    global rewards
    print(f'reward info requested by: {ctx.author.name}')
...
# yesInspection inspectionInspection
@slash.slash(name='uptime',
                description='Send bot uptime. Note that on heroku, dynos are cycled every 24 hours', guild_ids=guild_ids)
async def uptime(ctx):
    print(f'uptime requested by: {ctx.author.name}')
    """Tells you how long the bot has been up for."""
    await ctx.send('Uptime: **{}**'.format(get_bot_uptime()))


# noInspection WhatthehellisInspection
@slash.slash(name='instruction',
                description='Detailed instructions to get auth token and claim daily', guild_ids=guild_ids)
async def instruction(ctx):
    print(f'instruction requested by: {ctx.author.name}')
    embed = discord.Embed(title='How to use "STW Daily"', color=discord.Color.blurple())
    embed.set_footer(text='This bot was made by Dippy is not here', icon_url=ctx.author.avatar_url)
    embed.add_field(name='Welcome',
                    value='This will show you how to use the bot.\n\n'
                          ...
                          'the field of the stw daily command like so:\n``/daily authcode: a51c1f4d35b1457c8e34a1f6026faa35``'
...
@slash.slash(name='ping', description='Send websocket ping and embed edit latency', guild_ids=guild_ids)
async def ping(ctx):
    print(f'ping requested by: {ctx.author.name}')
    websocket_ping = '{0}'.format(int(client.latency * 100)) + ' ms'
    before = time.monotonic()
    # msg = await ctx.send("Pong!")
    # await msg.edit(content=f"Pong!  `{int(ping)}ms`")
    # await ctx.send(f'websocket latency: {client.latency*100}ms')
    # await ctx.send('websocket: {0}'.format(int(client.latency * 100)) + ' ms')
...
@slash.slash(name='emoji', description='Sends an animated loading emoji', guild_ids = guild_ids)
async def emoji(ctx):
    print(f'emoji requested by: {ctx.author.name}')
    # await ctx.send('<a:loading:759292972784418800>')
    # await ctx.send(f'<a:god:424520269315440650>')
    await ctx.send('<a:loadin:759293511475527760>')
...
async def daily(ctx, token=''):
    ...
    await ctx.defer()
    print(f'daily requested by: {ctx.author.name}')
    ...
    if token == "":
        ...
        if not gtResult[0]:
            ...
            if len(errorType) == 32:
                ...
                embed.add_field(
                    ...
                    value='This ctx may be a mistake then. If it is, let the owner know somehow. Join the'
        ...
        try:
            if str(r.text).find('{"errorCode":"') == '-1':
                ...
                if len(errorType) == 32:
                    ...
                    embed.add_field(
                        ...
                        inline=False)
                last_error_server = gtResult[1]
                print(str(r.text).split('{"errorCode":"', 1)[1].split('","errorctx":"', 1)[0])
                last_error_server = str(r.text).split('{"errorCode":"', 1)[1].split('","errorctx":"', 1)[0]
            else:
                try:
                    ...
                    try:
                        # await ctx.send(f'Debugging info because sometimes it breaks:\n{daily_feedback}')
            ...
            pass
        # embed.set_author(name=str(ctx.ctx.content)[9:],
